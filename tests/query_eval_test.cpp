...
std::string Encode2(const ValueLayout& layout, double cont,
                    const std::string& cat) {
  std::string out;
  EncodeValue(layout, {cont, cat}, "p", out);
  return out;
}
}  // namespace
...
TEST(QueryEval, EmptyQueryMatchesAll) {
  ...
  std::string out;
  EncodeValue(layout, {42.0}, "p", out);

  BitLSMQuery query;  // 빈 쿼리 → 항상 true
  EXPECT_TRUE(query.CheckCondition(rocksdb::Slice(out), layout));
}
...
TEST(QueryEval, OrderedGreaterEqual) {
  ...
  EXPECT_TRUE(
      query.CheckCondition(rocksdb::Slice(Encode2(layout, 15.0, "x")), layout));
  EXPECT_FALSE(
      query.CheckCondition(rocksdb::Slice(Encode2(layout, 5.0, "x")), layout));
  // >= 는 경계값 포함.
  EXPECT_TRUE(
      query.CheckCondition(rocksdb::Slice(Encode2(layout, 10.0, "x")), layout));
}
...
TEST(QueryEval, OrClauseSameAttribute) {
  ...
  EncodeValue(layout, {1.0}, "p", first_match);
  EXPECT_TRUE(query.CheckCondition(rocksdb::Slice(first_match), layout));

  std::string match;
  EncodeValue(layout, {2.0}, "p", match);
  EXPECT_TRUE(query.CheckCondition(rocksdb::Slice(match), layout));

  std::string no_match;
  EncodeValue(layout, {3.0}, "p", no_match);
  EXPECT_FALSE(query.CheckCondition(rocksdb::Slice(no_match), layout));
}
...
TEST(QueryEval, AndOfClausesMixedTypes) {
  ...
  EXPECT_TRUE(query.CheckCondition(
      rocksdb::Slice(Encode2(layout, 15.0, "apple")), layout));
  EXPECT_FALSE(query.CheckCondition(
      rocksdb::Slice(Encode2(layout, 15.0, "banana")), layout));
  EXPECT_FALSE(query.CheckCondition(
      rocksdb::Slice(Encode2(layout, 5.0, "apple")), layout));
}
  ...
  for (BitLSMQuery& q : queries) {
    ...
    for (double c : conts) {
      for (const char* s : cats) {
        std::string row = Encode2(layout, c, s);
        EXPECT_EQ(compiled.Eval(rocksdb::Slice(row)),
                  q.CheckCondition(rocksdb::Slice(row), layout))
            << q.ToString() << " on (" << c << ", '" << s << "')";
      }
    }
  }

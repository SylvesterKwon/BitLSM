...
// 목적: 컴파일된 라이브러리 심볼(BitLSM 생성자, bit_lsm.cpp)이 실제로 링크되는지 확인.
// create_if_missing=false(기본값)로 없는 경로를 열면 생성자가 throw 한다.
TEST(Smoke, BitLSMConstructorLinksAndThrowsOnBadOpen) {
  ...
  options.read_seqno = 0;
  options.rho = 0.5;
  EXPECT_THROW(bit_lsm::BitLSM("/nonexistent/bitlsm_smoke_dir", options,

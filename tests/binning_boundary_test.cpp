...
// Workload: 100 distinct continuous values but rho so small that many share a bin;
//           EQUAL queries after Flush.
// Threat: under-budgeted bins produce false-positive candidates that the MultiGet
...
TEST_F(BitLSMTestBase, CategoricalCardinalityExceedsBins) {
  ...
  ASSERT_TRUE(db.VerifyQuery(BitLSMQuery(std::vector<OrClause>{
      {{0, CompareOp::EQUAL, std::string("c7")},
       {0, CompareOp::EQUAL, std::string("c39")}}})));
}

// Workload: all rows share one continuous value (everything in one bin), then all
...
// Workload: a query that matches nothing and the empty query (matches everything).
// Threat: empty result set vs. full scan boundary handling in the iterator.
TEST_F(BitLSMTestBase, EmptyResultAndFullMatch) {
  ...
  for (int i = 0; i < 10; ++i)
    ASSERT_TRUE(db.Put("k" + std::to_string(i), {static_cast<double>(i)}, "p"));
  ASSERT_TRUE(db.Flush());
  ASSERT_TRUE(db.VerifyQuery(Cont(CompareOp::GREATER, 1000.0)));  // matches none
...
TEST_F(BitLSMTestBase, CnfContradictionAndOr) {
  ...
  ASSERT_TRUE(db.VerifyQuery(BitLSMQuery(std::vector<OrClause>{
      {{0, CompareOp::LESS, 3.0}, {0, CompareOp::GREATER, 17.0}},  // (a0<3 OR a0>17)

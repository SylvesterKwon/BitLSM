...
void create_kvp(DB* db, long long int n, int value_size = 32) {
    ...
    for (long long int cur_batch = 0; cur_batch < total_batch; ++cur_batch) {
        if (n - auto_increment < batch_size) kvps.resize(n - auto_increment);

        // Creating Batch KVPs
        for (size_t i = 0; i < kvps.size(); i++) {
            kvps[i] = {to_string(auto_increment++), random_string(value_size)};
        }

        // RocksDB Put
        WriteBatch batch;
        for (auto& [k, v] : kvps) batch.Put(k, v);
        db->Write(wo, &batch);
...
// SST 관찰 코드
void inspect_sst(const string& sst_file_path) {
    Options options;

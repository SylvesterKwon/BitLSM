...
public:
  ...
  vector<IndexSpec> specs = {
      {"user_id", (uint64_t)1e6, DistType::AUTO_INCREMENT},     // User Id
      {"gender", 2, DistType::UNIFORM},                         // Gender
      {"nationality", 250, DistType::UNIFORM},                  // Nationality
      {"income_level", 10, DistType::SCRAMBLED_ZIPFIAN, 0.99}}; // Income level
...
int main() {
  ...
  test_wg.Generate((uint64_t)1e6, 0.5, {0, 0, 2, 1}, {0, 1, 1, 1});
  return 0;
}